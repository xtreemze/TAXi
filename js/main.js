...
var check = function () {
    ...
    if (alive === false) {
        for (Step === 5; Step > 0; Step = Step - 5) {
            if (health < 1) {type("Ultimately, your poor mental and physical health have caused your body to fail. You survived for " +day +" days."); }
            if (food < 1) {type("You were unable to feed yourself and have died of starvation. You survived for " +day +" days."); }
            taxiDie();   
        } 
    } 
};
...
var passengerClick = function () {
    ...
    if (loaded === false && alive === true && wait === false && dist2cab < 1300) {
        ...
        $(this).animate({bottom: "+=20px"}, {duration:110}).animate({bottom: "-=20px"}, {duration:100}).animate({bottom: "+=16px"}, {duration:150}).animate({bottom: "-=16px"}, {duration:100});
        $(this).animate({right: "50px"}, {duration: dist2cab}).animate({bottom: "-=15px"}, {duration: 200}).fadeOut().animate({bottom: "+=15px"}, {duration: 1}).animate({right: "-200px"}, {duration:1});
    ...
    // What to do when taxi is occupied and passenger clicked on.
        ...
        if (stopped === true) {
            $(this).animate({bottom: "+=20px"}, {duration: 110}).animate({bottom: "-=20px"}, {duration: 100});
        // if the cab is occupied but not stopped
        } else {
            $(this).stop().animate({bottom: "110px", right:"-=23"}, {duration: 110,easing:"linear"}).animate({bottom: "90px", right:"-=22"}, {duration: 100, easing:"linear"}).animate({right:"-=20000px"},{duration:100000, easing:"linear"});
        }
    ...
    // What to do if taxi is clicked  vacant and not waiting for customers. If driving or stopped.
        ...
        if (stopped === false){
            $(this).stop().animate({bottom: "+=20px", right:"-=23"}, {duration: 110,easing:"linear"}).animate({bottom: "-=20px", right:"-=22"}, {duration: 100, easing:"linear"}).animate({right:"-=20000px"},{duration:100000, easing:"linear"});
        } else {
            $(this).animate({bottom: "+=20px"}, {duration: 110,easing:"linear"}).animate({bottom: "-=20px"}, {duration: 100, easing:"linear"});
        }
    ...
    if (wait === true) {
        ...
        // Function to drop off Passenger // Passenger drop       
            ...
            setTimeout(function(){
                ...
                wait = false;
                $(".passengerDrop").fadeOut(0).delay(200).css("right", "50px").css("bottom", "-=15px").fadeIn().animate({bottom: "+=15px"}, {duration: 200}).animate({right:"-=400px"},{duration:800}).stop();
...
var goLive = function () {
    for (var pedestrians = 5 ; pedestrians >= 0; pedestrians--) {
        ...
        $(".passenger"+[pedestrians]).animate({right:"-=20000px"},{duration:100000, easing:"linear"});
    } else if (stopped === false){
        $(".passenger"+[pedestrians]).css("right", Math.ceil(Math.random()*100)*50+1800+"px").fadeIn(0).animate({right:"-=20000px"},{duration:100000, easing:"linear"});
        }  
    }
};
...
var GoTaxi = function () {
    ...
    stopped = false;
    $(".background2").stop().animate({right:"-="+parseInt(225000/2.01, 10)}, 1000000,"linear" );
    var cubeArray = ['.storeCube', '.houseCube', '.officeCube', '.stationCube']; //Reposition buildings
...
var StopTaxi = function () {
    ...
    if (stopped === false){ // stops pedestrians, buildings and road from moving
      $(".background2").stop(true).animate({right:"-="+parseInt(112.5/2.01, 10)}, 500,"linear");
      $(".rider").stop().fadeIn(0).animate({right:"-=100px"},{duration:500, queue: false, easing:"linear"});
      $(".cubes").stop(true); cubeMove("-="+100*1.17+"px", 500);
      stopped = true;
...
var cubeMove = function(pixels, duration){
    $(".cubes").animate({right: pixels}, duration, "linear");
};
