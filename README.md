...
The runtime is three files plus repository-local assets:
...
The historical sprite/audio files remain in the repository for provenance, but the finished game does not depend on them. This removes the prototype's runtime reliance on jQuery, GSAP, Howler and third-party CDN URLs.
...
## GitHub Pages and visual capture
...
`.github/workflows/capture-gameplay.yml` is documentation-only automation. It launches Chromium against the same static root that Pages serves and refreshes the two README screenshots. The game itself has no Node dependency and does not need that workflow to run.
